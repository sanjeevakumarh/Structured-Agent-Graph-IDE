...
public sealed class WorkflowStepDispatcher
{
    ...
    private readonly IWorkflowStepRenderer _renderer;
    ...
    public WorkflowStepDispatcher(
        ...
        IWorkflowStepRenderer renderer,
    ...
    public async Task EvaluateNextStepsAsync(
    ...
    /// </summary>
    public async Task SubmitReadyStepsAsync(
    ...
    public async Task SubmitStepAsync(
        WorkflowStepDef stepDef, WorkflowInstance inst, WorkflowDefinition def, CancellationToken ct)
    {
        ...
        var resolvedPrompt = _renderer.RenderStep(
            basePrompt, inst.InputContext, inst.StepExecutions, 32_000);
        ...
        WorkflowStepDef stepDef, WorkflowInstance inst, WorkflowDefinition def)
    {
        ...
        var command    = _renderer.RenderStep(
            stepDef.Command ?? "", inst.InputContext, inst.StepExecutions, 32_000);
        var workingDir = string.IsNullOrWhiteSpace(stepDef.WorkingDir)
            ? inst.WorkspacePath ?? Directory.GetCurrentDirectory()
            : _renderer.RenderStep(
                stepDef.WorkingDir, inst.InputContext, inst.StepExecutions, 32_000);
